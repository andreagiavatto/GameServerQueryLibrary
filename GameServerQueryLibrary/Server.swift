...
// Changed from `class` to `struct` to eliminate the `@unchecked Sendable`
// suppression. As a value type, each actor that needs to work with a Server
// gets its own independent copy, so there is no shared mutable state and no
// data race. The `update` methods are marked `mutating` so call sites that
// need to modify a server create a local var copy and return the result —
// exactly the pattern already used by Q3InfoServer and Q3StatusServer.
